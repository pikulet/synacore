...
class Synacore:
    ...
    def __run_next_instruction(self):
        ...
        try:
            ...
            self.__logger.debug(
                self.__instruction_ptr - 1, "|", Opcode(opcode)
            )
            match Opcode(opcode):
                case Opcode.HALT:
    ...
    def __get_from_memory(self, address: int) -> bytearray:
        if address < MODULO_BASE:
            ...
            reg_value = Converter.BytesToInt(
                self.__registers.get(Converter.ValueToRegisterIndex(address))
            )
